...
# Для аутентификации будет использован email и пароль первого администратора (с наименьшим id)
...
#    request.set_form_data(entity: remote_entity_name, method: method_name, params: params.to_yaml)
    ...
    case response
      when Net::HTTPSuccess
        ...
        raise "Invalid response from remote entity(#{ response }). Address: #{ uri }, entity: #{ remote_entity_name }, method: #{ method_name }, params: #{ params.except(:email,:pwd).to_s }"
    end
  end
